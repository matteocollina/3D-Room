<script lang="ts">
	...
	let selectedImage = $state<{ name: string; url: string } | null>(null);

	function handleSelectImage(image: { name: string; url: string }) {
		selectedImage = image;
	}
</script>
...
<div class="text-base-900 dark:text-base-50 max-w-xl p-4">
	...
	{#if selectedImage}
		<div class="mt-4">
			<img src={selectedImage.url} alt="Selected from IndexedDB" />
		</div>
	{/if}
</div>
